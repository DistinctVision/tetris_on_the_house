...
uniform highp float edges_power;

uniform highp vec4 plane;
uniform highp float plane_edge_distance;
uniform highp float max_edges_distance;
...
{
    highp vec4 K = highp vec4(0.0, -1.0 / 3.0, 2.0 / 3.0, -1.0);
    highp vec4 p = mix(highp vec4(c.bg, K.wz), highp vec4(c.gb, K.xy), step(c.b, c.g));
    highp vec4 q = mix(highp vec4(p.xyw, c.r), highp vec4(c.r, p.yzx), step(p.x, c.r));
...
{
    highp vec4 K = highp vec4(1.0, 2.0 / 3.0, 1.0 / 3.0, 3.0);
...
highp vec3 changeColor(highp vec3 c)
{
    ...
    c1 = highp vec3(fract(c1.x * color_a.x + color_b.x),
